...
SELECT id, occurred_at AS time, total_amt_usd AS total_amount
FROM orders
...
and total dollar amount for each order, but this time sorted first by 
...
/* Companies whose name does not start with C */
SELECT name
FROM accounts
WHERE name NOT LIKE ('C%');

/*Companies whose name does not contain 'one'*/
SELECT name
FROM accounts
WHERE name NOT LIKE ('%one%');

/*Companies whose name does not end in s */
SELECT name
FROM accounts
...
/* Write a query that displays the order date and gloss_qty data 
